...
app.post("/api/exercises", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json(work_out.rows[0]);
  } catch (error) {
...
app.post("/api/sessions", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json(created_session);
  } catch (error) {
...
app.get("/users/:id/last-workout", async (req, res) => {
  const { id } = req.params;
  try {
...
app.get("/users/last-workouts", async (req, res) => {
  try {
...
app.get("/users/last-workouts-fixed", async (req, res) => {
  try {
    const all_users = await pool.query(
